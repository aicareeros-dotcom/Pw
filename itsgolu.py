...
def get_pssh_from_mpd(mpd_url):
    """Extract Widevine PSSH base64 from an MPD manifest URL."""
    resp = requests.get(mpd_url, timeout=20)
    resp.raise_for_status()
    root = ET.fromstring(resp.content)
    WIDEVINE_UUID = 'urn:uuid:edef8ba9-79d6-4ace-a3c8-27dcd51d21ed'
    for cp in root.iter():
        if 'ContentProtection' in cp.tag:
            if cp.get('schemeIdUri', '').lower() == WIDEVINE_UUID:
                for child in cp:
...
def get_license_url_from_mpd(mpd_url):
    """Try to extract the Widevine license URL from the MPD manifest."""
    try:
        resp = requests.get(mpd_url, timeout=20)
        resp.raise_for_status()
        root = ET.fromstring(resp.content)
        ...
        for cp in root.iter():
            if 'ContentProtection' in cp.tag:
                if cp.get('schemeIdUri', '').lower() == WIDEVINE_UUID:
                    for child in cp:
                        tag = child.tag.lower()
                        if 'laurl' in tag or 'licenseurl' in tag:
                            return child.text.strip() if child.text else None
                        la = child.get('licenseUrl') or child.get('laUrl') or child.get('laurl')
                        if la:
                            return la
    except Exception:
        pass
...
def get_keys_from_cdrm(pssh, license_url, license_headers=None):
    """Get Widevine decryption keys from cdrm-project.com public API."""
    if license_headers is None:
        license_headers = {}
    payload = {
    ...
    if data.get("status") != "success":
        raise ValueError(f"CDRM returned: {data.get('message', str(data))}")
...
async def decrypt_and_merge_video(mpd_url, keys_string, output_path, output_name, quality="720"):
    try:
        ...
        cmd1 = f'yt-dlp -f "bv[height<={quality}]+ba/b" -o "{output_path}/file.%(ext)s" --allow-unplayable-formats --no-check-certificate "{mpd_url}"'
        ...
        cmd4 = f'ffmpeg -i "{output_path}/video.mp4" -i "{output_path}/audio.m4a" -c copy "{output_path}/{output_name}.mp4" -y'
...
async def download_video(url, cmd, name):
    ...
    while retry_count < max_retries:
        ...
        download_cmd = f'{cmd} -R 25 --fragment-retries 25 -N 16'
        print(download_cmd)
    ...
    try:
        if os.path.isfile(name):
            ...
            if os.path.isfile(f"{name}.mkv"):
                ...
                found = f"{name}.mp4.webm"
            else:
                found = name + ".mp4"

        # Universal mkv → mp4 conversion
        if found.endswith(".mkv") and os.path.isfile(found):
            mp4_out = found[:-4] + ".mp4"
            ret = subprocess.run(
                f'ffmpeg -i "{found}" -c copy "{mp4_out}" -y',
                shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            if ret.returncode == 0 and os.path.isfile(mp4_out):
                os.remove(found)
                found = mp4_out
...
async def send_vid(bot: Client, m: Message, cc, filename, thumb, name, prog, channel_id, watermark="ONeX", topic_thread_id: int = None):
    try:
        ...
        # Ensure MP4 is streaming-ready (faststart moves metadata to start so Telegram shows inline)
        if filename.endswith(".mp4") and os.path.isfile(filename):
            faststart_out = filename.replace(".mp4", "_fs.mp4")
            fs_ret = subprocess.run(
                f'ffmpeg -i "{filename}" -c copy -movflags +faststart "{faststart_out}" -y',
                shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            if fs_ret.returncode == 0 and os.path.isfile(faststart_out):
                os.remove(filename)
                os.rename(faststart_out, filename)
