...
BUTTONSCONTACT = InlineKeyboardMarkup([[InlineKeyboardButton(text="📞 Contact", url="https://t.me/@Exter_pd62")]])
keyboard = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(text="🛠️ Help", url="https://t.me/Exter_pd62")        ],
    ]
)
    ...
    try:
        if m.chat.type == "channel":
            ...
            if not is_authorized:
                await m.reply_photo(
                    photo=photologo,
                    caption="**Mʏ Nᴀᴍᴇ [ONeX 🦋](https://t.me/Exter_pd62)\n\nYᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴀᴄᴄᴇꜱꜱ ᴛᴏ ᴜꜱᴇ ᴛʜɪꜱ ʙᴏᴛ\nCᴏɴᴛᴀᴄᴛ [ONeX](https://t.me/Exter_pd62) ғᴏʀ ᴀᴄᴄᴇꜱꜱ**",
                    reply_markup=InlineKeyboardMarkup([
    [
        InlineKeyboardButton("ONeX.™®", url="https://t.me/@Exter_pd62")
    ],
    [
        ...
        InlineKeyboardButton("ᴅᴇᴛᴀɪʟꜱ 🦋", callback_data="details")
            ...
            commands_list = (
                "**>  /txt - ꜱᴛᴀʀᴛ ᴜᴘʟᴏᴀᴅɪɴɢ ᴄᴘ/ᴄᴡ ᴄᴏᴜʀꜱᴇꜱ**\n"
...
@bot.on_message(filters.command(["txt"]) & auth_filter)
async def txt_handler(bot: Client, m: Message):  
        ...
        for i in content:
            if "://" in i:
                ...
                if ".pdf" in url:
                    ...
                    yt_count += 1
                elif "raonetech.online/get/appx/signedUrl" in url:
                    m3u8_count += 1
                elif "raonetech.online/get/appx-pdf/signedUrl" in url:
                    pdf_count += 1
                elif "zip" in url:
                    zip_count += 1
                else:
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            appx_xor_key = ""  # XOR decryption key for appx encrypted videos
            if "raonetech.online/get/appx/signedUrl" in url:
                try:
                    appx_cdn_headers = {
                        'User-Agent': 'Chrome Mobile',
                        'Referer': 'https://apnitaiyariapp.akamai.net.in/',
                        'Origin': 'https://lakshyaclasses.appx.co.in',
                    }
                    async with aiohttp.ClientSession() as session:
                        async with session.get(url) as resp:
                            data = await resp.json(content_type=None)
                            resolved = data.get("link", url)
                            # Save XOR key for decryption later
                            import base64 as _b64
                            _raw_key = data.get("key", "")
                            if _raw_key:
                                try:
                                    appx_xor_key = _b64.b64decode(_raw_key).decode()
                                except Exception:
                                    appx_xor_key = _raw_key
                            # Verify URL is accessible with appx headers
                            try:
                                async with session.head(resolved, headers=appx_cdn_headers, timeout=aiohttp.ClientTimeout(total=10)) as hresp:
                                    if hresp.status < 400:
                                        url = resolved
                                    else:
                                        await bot.send_message(channel_id, f"⚠️ <b>Skipped (CDN {hresp.status})</b> — <code>{str(count).zfill(3)}) {name1}</code>\n<blockquote>Content not available on appx CDN.</blockquote>", disable_web_page_preview=True)
                                        count += 1
                                        failed_count += 1
                                        continue
                            except Exception:
                                url = resolved  # proceed anyway, let download handle failure
                except Exception as _e:
                    await bot.send_message(channel_id, f"⚠️ Failed to resolve signedUrl for <b>{name1}</b>: {_e}", disable_web_page_preview=True)
                    count += 1
                    failed_count += 1
                    continue

            if "raonetech.online/get/appx-pdf/signedUrl" in url:
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(url, headers={
                            ...
                            if resp.status == 307:
                                ...
                                # static-db-v2 works with Chrome Mobile headers — no domain swap needed
                                # only swap truly broken domains
                                cdn_url = cdn_url.replace('static-wsb.appx.co.in', 'appx-content-v2.classx.co.in')
                                cdn_url = cdn_url.replace('static-db.appx.co.in', 'appxcontent.kaxa.in')
                                url = cdn_url
                            else:
                                data = await resp.json(content_type=None)
                                url = data.get("link", url)
                except Exception as _e:
                    await bot.send_message(channel_id, f"⚠️ Failed to resolve PDF signedUrl for <b>{name1}</b>: {_e}", disable_web_page_preview=True)
                    count += 1
                    failed_count += 1
                    continue

            if "acecwply" in url:
                cmd = f'yt-dlp -o "{name}.mp4" -f "bestvideo[height<={raw_text2}]+bestaudio" --hls-prefer-ffmpeg --no-keep-video --merge-output-format mp4 --no-warning "{url}"'
                ...
                if len(parts_split) < 2:
                    ...
                    failed_count += 1
                    continue
                base_with_params, signature = parts_split
                ...
                parts_split2 = url.split("*", 1)
                if len(parts_split2) < 2:
                    count += 1
                    failed_count += 1
                    continue
                base_with_params, signature = parts_split2
            ...
            if "jw-prod" in url:
               ...
               cmd = f'yt-dlp --add-header "referer:https://web.classplusapp.com/" --add-header "x-cdn-tag:empty" -f "{ytf}" "{url}" -o "{name}.mp4"'
                ...
                cmd = (
                    f'yt-dlp'
                    f' --add-header "User-Agent:Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36"'
                    f' --add-header "Referer:https://lakshyaclasses.appx.co.in/"'
                    f' --add-header "Origin:https://lakshyaclasses.appx.co.in"'
    ...
    f"\n\n<b>🎓  Uᴘʟᴏᴀᴅ Bʏ : {CR}</b>"
                ...
                if "drive" in url:
                    ...
                    if "cwmediabkt99" in url:
                        ...
                        "static-wsb.appx.co.in",
                        "appx-content-v2.classx.co.in",
                        "appxcontent.kaxa.in",
                        "static-db-v2.appx.co.in",
                        "static-db.appx.co.in",
                        "static-trans-v2.appx.co.in",
                        "static-trans-v1.appx.co.in",
                    ]):
                        try:
                            import subprocess as _sp

                            def _curl_download(dl_url, dest_path):
                                cmd = [
                                    "curl", "-L", "--silent", "--show-error",
                                    "--retry", "5", "--retry-delay", "3",
                                    "--connect-timeout", "20", "--max-time", "600",
                                    "-H", "User-Agent: Chrome Mobile",
                                    "-H", "Referer: https://apnitaiyariapp.akamai.net.in/",
                                    "-H", "Origin: https://lakshyaclasses.appx.co.in",
                                    "-o", dest_path, dl_url
                                ]
                            ...
                            # For old appx domains (paid_course2/ path) try mirror swap
                            if "static-wsb.appx.co.in" in url:
                                from urllib.parse import urlparse
                                p = urlparse(url)
                                mirror_url = url.replace("static-wsb.appx.co.in", "appx-content-v2.classx.co.in")
                                rc = await _asyncio.to_thread(_curl_download, mirror_url, f'{name}.pdf')
                                if rc != 0 or not os.path.exists(f'{name}.pdf') or os.path.getsize(f'{name}.pdf') < 100:
                                    await bot.send_message(channel_id,
                                        f"⚠️ <b>PDF Skipped</b> — <code>{name1}</code>\n"
                                        f"<blockquote>File deleted from Appx servers (path: <code>{p.path[:60]}</code>)</blockquote>",
                                        disable_web_page_preview=True)
                                    failed_count += 1
                                    count += 1
                                    continue
                            else:
                                rc = await _asyncio.to_thread(_curl_download, url, f'{name}.pdf')
                                if rc != 0 or not os.path.exists(f'{name}.pdf') or os.path.getsize(f'{name}.pdf') < 100:
                                    raise Exception(f"curl exited with code {rc}")
                            copy = await bot.send_document(chat_id=channel_id, document=f'{name}.pdf', caption=cc1)
                            count += 1
                            os.remove(f'{name}.pdf')
                        except FloodWait as e:
                    ...
                    Show = f"<i><b>📥 Encrypted Video Downloading</b></i>\n<blockquote><b>{str(count).zfill(3)}) {name1}</b></blockquote>"
                    prog = await bot.send_message(channel_id, Show, disable_web_page_preview=True)
                    try:
                        # appx/classx CDN
                        if any(d in url for d in ["appx.co.in", "classx.co.in", "appxcontent", "akamai.net.in"]):
                            if appx_xor_key:
                                # Appx 2-phase encryption:
                                #   bytes 0..klen-1      → XOR with ASCII key
                                #   bytes klen..4*klen-1 → XOR with position index i
                                #   bytes 4*klen+        → plain (no change)
                                import re as _re, aiohttp as _aiohttp
                                _safe_name = _re.sub(r'[^\w\-]', '_', name)[:80]
                                raw_mkv = f"{_safe_name}_enc.mkv"
                                # Download using Python aiohttp (handles special chars in URL safely)
                                try:
                                    async with _aiohttp.ClientSession() as _sess:
                                        async with _sess.get(url, headers={
                                            'User-Agent': 'Chrome Mobile',
                                            'Referer': 'https://apnitaiyariapp.akamai.net.in/',
                                            'Origin': 'https://lakshyaclasses.appx.co.in',
                                        }, timeout=_aiohttp.ClientTimeout(total=600)) as _resp:
                                            with open(raw_mkv, 'wb') as _f:
                                                async for _chunk in _resp.content.iter_chunked(1024*1024):
                                                    _f.write(_chunk)
                                    res_file = raw_mkv if (os.path.exists(raw_mkv) and os.path.getsize(raw_mkv) > 10240) else None
                                except Exception as _de:
                                    res_file = None
                                # Decrypt using 2-phase algorithm
                                if res_file and os.path.exists(res_file):
                                    _xor_key = appx_xor_key.encode()
                                    _klen = len(_xor_key)
                                    _enc_boundary = 4 * _klen
                                    dec_tmp = res_file + ".dec"
                                    _global_pos = 0
                                    with open(res_file, 'rb') as _fin, open(dec_tmp, 'wb') as _fout:
                                        while True:
                                            _chunk = _fin.read(65536)
                                            if not _chunk:
                                                break
                                            _out = bytearray(len(_chunk))
                                            for _ci in range(len(_chunk)):
                                                _gi = _global_pos + _ci
                                                if _gi < _klen:
                                                    _out[_ci] = _chunk[_ci] ^ _xor_key[_gi]
                                                elif _gi < _enc_boundary:
                                                    _out[_ci] = _chunk[_ci] ^ _gi
                                                else:
                                                    _out[_ci] = _chunk[_ci]
                                            _fout.write(bytes(_out))
                                            _global_pos += len(_chunk)
                                    os.remove(res_file)
                                    os.rename(dec_tmp, res_file)
                                # Convert decrypted mkv → mp4
                                if res_file and res_file.endswith(".mkv") and os.path.exists(res_file):
                                    mp4_out = res_file[:-4] + ".mp4"
                                    conv = subprocess.run(
                                        f'ffmpeg -i "{res_file}" -c copy -movflags +faststart "{mp4_out}" -y',
                                        shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE
                                    )
                                    if conv.returncode == 0 and os.path.exists(mp4_out) and os.path.getsize(mp4_out) > 1024:
                                        os.remove(res_file)
                                        res_file = mp4_out
                            else:
                                # No XOR key — video is CDN-signed only, download normally
                                enc_cmd = (
                                    f'yt-dlp'
                                    f' --add-header "User-Agent:okhttp/4.9.2"'
                                    f' --add-header "Referer:https://lakshyaclasses.appx.co.in/"'
                                    f' --add-header "Origin:https://lakshyaclasses.appx.co.in"'
                                    f' -f "{ytf}" -o "{name}.mp4"'
                                    f' "{url}"'
                                )
                                res_file = await helper.download_video(url, enc_cmd, name)
                        else:
                            if any(d in url for d in ["raonetech"]):
                                referer = "https://www.raonetech.online/"
                            else:
                                referer = "https://www.google.com/"
                            enc_cmd = (
                                f'yt-dlp'
                                f' --add-header "User-Agent:Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36"'
                                f' --add-header "Referer:{referer}"'
                                f' --add-header "Origin:{referer.rstrip("/")}"'
                                f' -f "{ytf}" "{url}" -o "{name}.mp4"'
                            )
                            res_file = await helper.download_video(url, enc_cmd, name)
                        filename = res_file
...
async def back_to_start_callback(client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_media(
        ...
        reply_markup=InlineKeyboardMarkup([
            [
                InlineKeyboardButton("𝐈𝐓'𝐬𝐆𝐎𝐋𝐔.™®", url="https://t.me/Exter_pd62")
            ],
            [
                InlineKeyboardButton("ғᴇᴀᴛᴜʀᴇꜱ 🪔", callback_data="features"),
                InlineKeyboardButton("ᴅᴇᴛᴀɪʟꜱ 🦋", callback_data="details")
...
for _attempt in range(_max_retries):
    try:
        ...
        print(f"[FloodWait] Telegram ne {_fw.value}s wait maanga. Sone ja raha hoon...")
        _time.sleep(_fw.value + 5)
        print(f"[FloodWait] Retry attempt {_attempt+1}/{_max_retries}...")
    except Exception as _e:
        print(f"[ERROR] Bot crash: {_e}")
        break
